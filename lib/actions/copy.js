...
async function applyProcessingFunc(process, contents, filename) {
  var output = await process(contents, filename);
  return output instanceof Buffer ? output : Buffer.from(output);
}

exports.copy = async function (from, to, options, context, tplSettings) {
  to = path.resolve(to);
  options = options || {};
  ...
  for (const file of files) {
    await this._copySingle(file, generateDestination(file), options, context, tplSettings);
  }
};

exports._copySingle = async function (from, to, options, context, tplSettings) {
  options = options || {};
